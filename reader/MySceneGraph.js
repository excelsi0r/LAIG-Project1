...
MySceneGraph.prototype.parsePrimitives = function(rootElement)
{
	...
	else
	{
		for (var i = 0; i < nprimitives; i++)
		{
			...
			{
				var n_surfaces = primitive[i].children[0].children.length;
				var surfaces = primitive[i].children[0].children;

				primitiveitem['surfaces'] = [];

				for(var tr = 0; tr < n_surfaces; tr++)
				{
					var surface = [];

					var type = surfaces[tr].tagName;		

					if(type == 'plane')
					{
						surface['id'] = type;

						var dimx = parseFloat(surfaces[tr].getAttribute('dimX'));
						var dimy = parseFloat(surfaces[tr].getAttribute('dimY'));
						var partsx = parseInt(surfaces[tr].getAttribute('partsX'));
						var partsy = parseInt(surfaces[tr].getAttribute('partsY'));

						if(dimx <= 0 || dimy <= 0 || partsx <= 0 || partsy <=0)
						{
							return "Primitive'" + id + "' of type vehicle, has plane that can't have bellow 0 components";
						}

						surface['dimX'] = dimx;
						surface['dimY'] = dimy;
						surface['partsX'] = partsx;
						surface['partsY'] = partsy;
						primitiveitem['surfaces'].push(surface);
					...
					else if(type == 'patch')
					{
						...
						var orderu = parseInt(surfaces[tr].getAttribute('orderU'));
						var orderv = parseInt(surfaces[tr].getAttribute('orderV'));
						var partsu = parseInt(surfaces[tr].getAttribute('partsU'));
						var partsv = parseInt(surfaces[tr].getAttribute('partsV'));

						surface['orderU'] = orderu;
						surface['orderV'] = orderv;
						surface['partsU'] = partsu;
						surface['partsV'] = partsv;

						if(orderu < 1 || orderu > 3 || orderv < 1 || orderv > 3)
						{
							return "Primitive'" + id + "' of vehilce, has a patch that must have orders 1, 2 or 3";
						}

						if(partsu < 1 || partsv < 1)
						{
							return "Primitive'" + id + "' of type vehicle, has a patch which parts must be bigger than 1";
						}

						surface['controlpoints'] = [];


						var controlpoints = surfaces[tr].children; 
						var n_controlpoints = surfaces[tr].children.length;

						if(n_controlpoints != (orderu + 1)*(orderv + 1))
						{
							return "Primtive '" + id+ "' of type vehicle, has a patch that contains unexpected number of control points. Expected: " + (orderu + 1)*(orderv + 1) + ". Got: " + n_controlpoints + "."; 
						}
